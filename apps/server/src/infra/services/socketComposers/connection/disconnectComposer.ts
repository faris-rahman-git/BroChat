...
import { DisconnectUseCase } from '../../../../app/socketUseCase/connection/implementations/DisconnectUseCase';
import { IDisconnectUseCase } from '../../../../app/socketUseCase/connection/interfaces/IDisconnectUseCase';
import { disconnectController } from '../../../../presentation/socket/controllers/connection/disconnectController';

export function disconnectComposer(): ISocketController {
  ...
  const useCase: IDisconnectUseCase = new DisconnectUseCase(
    userManagementRepo,
    eventQueueService
  );

  const controller: ISocketController = new disconnectController(useCase);
  return controller;
}

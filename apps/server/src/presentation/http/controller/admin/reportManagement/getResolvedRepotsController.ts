...
export class getResolvedRepotsController implements IController {
  constructor(
    private getResolvedRepotsUseCase: IGetResolvedRepotsUseCase,
  ...
  async handle(httpRequest: IHttpRequest): Promise<IHttpResponse> {
    ...
    try {
      ...
      response = await this.getResolvedRepotsUseCase.execute(searchValue, page);

      const success = this.httpSuccess.success_200(response.data);
      return new HttpResponse(success.statusCode, success.body);
    } catch (err) {
      console.error('Error in getResolvedRepotsController:', error);

...
export class ignoreReportController implements IController {
  constructor(
    private ignoreReportUseCase: IIgnoreReportUseCase,
  ...
  async handle(httpRequest: IHttpRequest): Promise<IHttpResponse> {
    ...
    try {
      ...
      response = await this.ignoreReportUseCase.execute(reportId, note);

      const success = this.httpSuccess.success_200(response.data);
      return new HttpResponse(success.statusCode, success.body);
    } catch (err) {
      console.error('Error in ignoreReportController:', error);

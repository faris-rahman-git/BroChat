...
export class softDeleteUserController implements IController {
  constructor(
    private softDeleteUserUseCase: ISoftDeleteUserUseCase,
  ...
  async handle(httpRequest: IHttpRequest): Promise<IHttpResponse> {
    ...
    try {
      const { userId, searchValue, status, joinedAt, page } =
        httpRequest.body as {
          userId: string;
          searchValue: string;
          status: string;
      ...
      response = await this.softDeleteUserUseCase.execute(
        userId,
        searchValue,
        status,
        joinedAt,

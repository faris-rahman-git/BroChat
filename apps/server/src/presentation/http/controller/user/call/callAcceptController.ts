...
export class callAcceptController implements IController {
  constructor(
    private callAcceptUseCase: ICallAcceptUseCase,
  ...
  async handle(httpRequest: IHttpRequest): Promise<IHttpResponse> {
    ...
    try {
      ...
      const data = httpRequest.body as acceptCallApiType;

      response = await this.callAcceptUseCase.execute(userId, data);

      const success = this.httpSuccess.success_200(response.data);
      return new HttpResponse(success.statusCode, success.body);
    } catch (err) {
      console.error('Error in callAcceptController:', error);

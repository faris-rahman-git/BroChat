...
export class createNewConversationController implements IController {
  constructor(
    private createNewConversationUseCase: ICreateNewConversationUseCaseUseCase,
  ...
  async handle(httpRequest: IHttpRequest): Promise<IHttpResponse> {
    ...
    try {
      ...
      const { receiverId } = httpRequest.body as { receiverId: string };

      response = await this.createNewConversationUseCase.execute(
        userId,
        receiverId
      );

...
export class oneToOneChatListController implements IController {
  constructor(
    private oneToOneChatListUseCase: IOneToOneChatListUseCase,
  ...
  async handle(httpRequest: IHttpRequest): Promise<IHttpResponse> {
    ...
    try {
      ...
      response = await this.oneToOneChatListUseCase.execute(userId);

      const success = this.httpSuccess.success_200(response.data);
      return new HttpResponse(success.statusCode, success.body);
    } catch (err) {
      console.error('Error in oneToOneChatListController:', error);

...
export class exitFromGroupController implements IController {
  constructor(
    private exitFromGroupUseCase: IExitFromGroupUseCase,
  ...
  async handle(httpRequest: IHttpRequest): Promise<IHttpResponse> {
    ...
    try {
      ...
      response = await this.exitFromGroupUseCase.execute(
        conversationId,
        userId
      );
      ...
      console.error('Error in exitFromGroupController:', error);
